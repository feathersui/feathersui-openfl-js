...
let srcDirPath = _path.resolve(
  baseDir,
  _path.join("node_modules", "feathersui-openfl-haxelib", "src")
);
...
async function start(option) {
  ...
  if (option == "fix-npm-libs") {
    ...
    output(
      `\nComplete! ${totalFilesAddedOrModified} files were created or modified`,
      true
    );
    process.exit(0);
    ...
    output(
      "\nOption unrecognized or not specified. Nothing to be done. Exiting script.",
      true
    );
    process.exit(1);
  }
...
function init() {
  class DelayFS {
    ...
    updateOrAddToVirtualFileSystem(fullFilePath, content, options) {
      ...
      if (
        options.readFromRealFileSystem != null &&
        options.readFromRealFileSystem
      ) {
    ...
    readFileSyncUTF8(filePath, extra) {
      ...
      if (typeof extra == "string") {
        ...
        if (extra.tags != null) tags = extra.tags;
      }

      this.updateOrAddToVirtualFileSystem(fullFilePath, content, {
        readFromRealFileSystem: true,
        tags: tags,
      });
    ...
    writeFileSyncUTF8(filePath, content, extra) {
      ...
      if (typeof extra == "string") {
        ...
        if (extra.createOnly != null) createOnly = extra.createOnly;
        else if (extra.tags != null) tags = extra.tags;
      }
    ...
    getIgnored(tags) {
      ...
      return this.unmodified
        .filter((value) => {
          return (
            value.tags &&
            value.tags.some((value) => value == tags) &&
            value.ignored
          );
        })
        .map((value) => value.path);
...
function skipHxPath(path) {
  ...
  return (
    content.indexOf(`abstract ${symbolName}(`) !== -1 ||
    content.indexOf(`abstract ${symbolName}<`) !== -1
  );
}
...
function startCreateLibCjs() {
  return globby([
    normalizePath(_path.resolve(srcFeathersDirPath, `**/*.hx`)),
  ]).then((paths) => {
    for (let path of paths) {
      ...
      let cjsFilePath = path
        .replace(srcDirPath + "/", libCjsDirPath + "/")
        .replace(".hx", ".js");
      let cjsGenFilePath = cjsFilePath.replace(
        libCjsDirPath + "/",
        libCjsGenDirPath + "/"
      );
      ...
      let content = _fs.readFileSync(path, "utf8");
      let findClasses = /^class (\w+)/gm;
      let foundOtherClasses;
...
function startCreateLibCjsIndexModules() {
  return globby([
    normalizePath(_path.resolve(libCjsDirPath, "feathers", "**/*.js")),
  ]).then((paths) => {
...
function createPackageIndexLine(indexDirPath, packageDirPath) {
  ...
  let relativePath = normalizePath(
    _path.relative(indexDirPath, packageDirPath)
  );
...
function createLibCjsModule(cjsPath, cjsGenPath) {
  let relativePath = normalizePath(
    _path.relative(_path.dirname(cjsPath), cjsGenPath)
  );
  relativePath = relativePath.substr(0, relativePath.length - 3);
